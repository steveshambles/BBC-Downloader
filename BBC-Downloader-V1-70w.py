"""BBC Downloader V1.70W

By Steve Shambles Nov 2019 and Feb 2020
...
BBC Downloader was built using get_iplayer v3.22, other versions
of get_iplayer may or may not work.
----------------------------------------------------

V1.66-70 - Added quality control menus to  allow diff quality dowloads.
...
root = Tk()
root.title('BBC Downloader V1.70W.')
...
class Glo():
    ...
    radq = 'good'     # Default radio download quality as global
    tvq = 'worst'     # Default tv download quality as global
...
def dwnld_show():
    ...
    if not clpbrd_url.startswith('https://www.bbc.co.uk/'):
        messagebox.showerror('Error',
                             'This does not look like a valid BBC URL')
        return

    ask_yn = messagebox.askyesno('Question',
                                 'Download show from pasted URL?')
    if ask_yn is False:
        return
    ...
    # Execute get_iplayer command if radio
    if not 'iplayer' in clpbrd_url:
        run_cmd = r'C:\Windows\System32\cmd.exe /kget_iplayer.cmd '  \
                   +str(clpbrd_url)+' --radiomode='+str(Glo.radq)
        subprocess.Popen(run_cmd)

    # Execute get_iplayer command if tv
    if 'iplayer' in clpbrd_url:
        run_cmd = r'C:\Windows\System32\cmd.exe /kget_iplayer.cmd '  \
                   +str(clpbrd_url)+' --tvmode='+str(Glo.tvq)
        subprocess.Popen(run_cmd)
...
def about_menu():
    ...
    messagebox.showinfo('Program Information', 'BBC Downloader V1.70W\n'
                        'Freeware. By Steve Shambles, Feb 2020\n\n'
...
def help_menu():
    ...
    messagebox.showinfo('How To...', 'Download BBC Shows V1.70W\n\n'
                        ...
                        '06: If desired, choose quality of download from quality menu.\n\n'
                        '07: Click the "Download" button.\n\n'
                        '08: After download, click "Refresh Recordings List" button.\n\n'
                        '09: Double click on a file to play it.\n\n'
                        '10: Right click on a file to convert it to mp3 if req.\n\n'
                        '11: You can set the quality of the Mp3 from the "Bitrate menu".\n\n'
                        '12: TV show audio stream can also be converted to Mp3 if so desired.\n\n'
                        '13: Click "Open recordings folder" to edit files.\n\n'
...
menu_bar.add_cascade(label='Quality', menu=file_menu3)

submenu3 = Menu(menu_bar, tearoff=0)
file_menu3.add_cascade(label='Convert to Mp3 bitrate:', menu=submenu3)
submenu3.add_checkbutton(label='  32kbps - Speech', variable=br_32,
                           command=convert_32)
submenu3.add_checkbutton(label='  64kbps - Speech', variable=br_64,
                           command=convert_64)
submenu3.add_checkbutton(label='128kbps - Speech', variable=br_128,
                           command=convert_128)
submenu3.add_checkbutton(label='160Kbps - Music', variable=br_160,
                           command=convert_160)
submenu3.add_checkbutton(label='192Kbps - Music', variable=br_192,
                           command=convert_192)
submenu3.add_checkbutton(label='256Kbps - Music', variable=br_256,
                           command=convert_256)
submenu3.add_checkbutton(label='320Kbps - Music', variable=br_320,
                           command=convert_320)
...
def tv_qlty_worst():
    """Set download quality of TV show to lowest quality."""
    Glo.tvq = 'worst'
    tvq_worst.set(1)
    tvq_good.set(0)
    tvq_better.set(0)
...
def tv_qlty_good():
    """Set download quality of TV show to next best quality."""
    Glo.tvq = 'good'
    tvq_worst.set(0)
    tvq_good.set(1)
    tvq_better.set(0)
...
def tv_qlty_better():
    """Set download quality of TV show to next better quality."""
    Glo.tvq = 'better'
    tvq_worst.set(0)
    tvq_good.set(0)
    tvq_better.set(1)
...
def tv_qlty_best():
    """Set download quality of TV show to best quality available."""
    Glo.tvq = 'best'
    tvq_worst.set(0)
    tvq_good.set(0)
    tvq_better.set(0)
...
file_menu3.add_cascade(label='TV download quality', menu=submenu)
submenu.add_checkbutton(label='Worst', variable=tvq_worst, command=tv_qlty_worst)
submenu.add_checkbutton(label='Good', variable=tvq_good, command=tv_qlty_good)
submenu.add_checkbutton(label='Better', variable=tvq_better, command=tv_qlty_better)
submenu.add_checkbutton(label='Best', variable=tvq_best, command=tv_qlty_best)

radq_worst = IntVar()
radq_good = IntVar()
radq_better = IntVar()
radq_best = IntVar()
radq_good.set(1) # Default radio download quality

def rad_qlty_worst():
    """Radio show download quality - 48kbs"""
    Glo.radq = 'worst'
    radq_worst.set(1)
    radq_good.set(0)
    radq_better.set(0)
...
def rad_qlty_good():
    """Radio show download quality - 96kbs"""
    Glo.radq = 'good'
    radq_worst.set(0)
    radq_good.set(1)
    radq_better.set(0)
...
def rad_qlty_better():
    """Radio show download quality - 128kbs"""
    Glo.radq = 'better'
    radq_worst.set(0)
    radq_good.set(0)
    radq_better.set(1)
...
def rad_qlty_best():
    """Radio show download quality - 320kbs"""
    Glo.radq = 'best'
    radq_worst.set(0)
    radq_good.set(0)
    radq_better.set(0)
...
file_menu3.add_cascade(label='Radio download quality', menu=submenu2)
submenu2.add_checkbutton(label='Worst', variable=radq_worst, command=rad_qlty_worst)
submenu2.add_checkbutton(label='Good', variable=radq_good, command=rad_qlty_good)
submenu2.add_checkbutton(label='Better', variable=radq_better, command=rad_qlty_better)
submenu2.add_checkbutton(label='Best', variable=radq_best, command=rad_qlty_best)
